#!/bin/bash
#SBATCH --job-name=ppg_prs_report
...
ppg_name="lora-prs-continuation-v1_${mode}_report_seed${seed}_job${SLURM_JOB_ID}"
ppg_output="$ppg_work/outputs/$ppg_name"
if [[ "$mode" == final ]]; then
  python -m pulsedb_fewshot.lora_prs_report --random-report "$1" --chronological-report "$2" --output "$ppg_output"
else
  arguments=()
