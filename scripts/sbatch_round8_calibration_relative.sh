#!/bin/bash
#SBATCH --job-name=ppg_r8_relative
...
python -m pulsedb_fewshot.round8_calibration_relative train \
  --prepared "$prepared" \
  --output "$output" \
  --method "$method" \
  --seed "$seed"
...
echo "ROUND8_RELATIVE_COMPLETE=$run_id"
