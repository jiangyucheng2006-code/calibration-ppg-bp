...
cd "$ppg_project"
ppg_name="lora-prs-continuation-v1_${split_mode}_${candidate}_seed${seed}_job${SLURM_JOB_ID}"
ppg_source="$ppg_work/outputs/same-subject-personal-profile-v1_${split_mode}_subject_lora_rank4_seed20260904_job1399"
if [[ "$split_mode" == chronological_blocked ]]; then
  ppg_source="$ppg_work/outputs/same-subject-personal-profile-v1_${split_mode}_subject_lora_rank4_seed20260904_job1408"
fi
python -m pulsedb_fewshot.lora_prs_train --source-run "$ppg_source" --candidate "$candidate" \
