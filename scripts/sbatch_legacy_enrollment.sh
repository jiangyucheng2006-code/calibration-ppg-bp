...
#SBATCH --output=/home/jiangyu.cheng/work/ppg_bp/logs/legacy_enrollment_%j.log
set -euo pipefail
umask 077
...
ppg_store="${3:?audited source pool}"
[[ "$ppg_root" == "$ppg_work/outputs/legacy-split-enrollment-v1_"* ]]
...
archive() {
  ...
  cp "$ppg_work/logs/legacy_enrollment_${SLURM_JOB_ID}.log" "$ppg_nas/logs/" || true
  exit "$code"
}
trap archive EXIT
cd "$ppg_code"
case "$ppg_stage" in
  smoke)
    mkdir "$ppg_output"
    "$ppg_python" -m unittest discover -s tests -p 'test_legacy_enrollment.py' -v
    ...
    "$ppg_python" -c 'import os,sys; from pathlib import Path; from pulsedb_fewshot.official_calbased_train import save_json; from pulsedb_fewshot.training import source_tree_sha256; save_json(Path(sys.argv[1]),{"status":"pass","protocol_id":"legacy-split-enrollment-v1","snapshot":os.environ["PPG_PROJECT_ROOT"],"source_store":sys.argv[2],"source_tree_sha256":source_tree_sha256(Path(os.environ["PPG_PROJECT_ROOT"])),"slurm_job_id":os.environ["SLURM_JOB_ID"],"gpu_test_passed":True})' "$ppg_output/receipt.json" "$ppg_store"
    ;;
  prepare)
    "$ppg_python" -m pulsedb_fewshot.legacy_enrollment_protocol --store-root "$ppg_store" --legacy-split "$ppg_split" --full-index "$ppg_index" --output "$ppg_output"
