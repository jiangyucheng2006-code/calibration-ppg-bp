...
ppg_mode="${2:-materialize}"
[[ "$ppg_mode" == materialize || "$ppg_mode" == preflight ]]
...
archive() {
  ...
  if [[ -f "$ppg_store/postprepare_audit.json" ]]; then
    cp "$ppg_store/postprepare_audit.json" "$ppg_nas/data/manifests/$(basename "$ppg_store")/"
  fi
...
ppg_options=(--use-verified-membership-cache)
if [[ "$ppg_mode" == preflight ]]; then ppg_options+=(--preflight-only); fi
"$ppg_python" -m pulsedb_fewshot.official_calbased_data \
  ...
  --output "$ppg_store" --workers 2 --shards 32 "${ppg_options[@]}"
if [[ "$ppg_mode" == materialize ]]; then
