...
[[ "$ppg_tag" =~ ^[0-9]{8}-[0-9]{6}$ ]]
ppg_root="$ppg_work/outputs/full-cohort-enrollment-v1_$ppg_tag"
...
submit() {
  ...
  local options=(--parsable --export=ALL --job-name="ppg_full_${stage}" --kill-on-invalid-dep=yes --dependency="afterok:$dependency" --time="$duration" --cpus-per-task="$cpus" --mem="$mem")
  [[ "$gpu" == none ]] || options+=(--gres="gpu:$gpu:1")
  id="$(sbatch "${options[@]}" "$ppg_code/scripts/sbatch_full_enrollment.sh" "$stage" "$ppg_root")"
  id="${id%%;*}"
  [[ "$id" =~ ^[0-9]+$ ]]
  printf '%s\t%s\t%s\t%s\n' "$stage" "$id" "$dependency" "$gpu" >> "$ppg_manifest"
  printf '%s' "$id"
}
materialize="$(submit materialize "$ppg_smoke" none 24:00:00 4 12G)"
prepare="$(submit prepare "$materialize" none 06:00:00 4 24G)"
population="$(submit population "$prepare" rtx_5080 72:00:00 4 24G)"
v0="$(submit validation_0 "$population" rtx_5080 48:00:00 2 12G)"
v1="$(submit validation_1 "$population" rtx_5070_ti 48:00:00 2 12G)"
validation="$(submit validation_score "$v0:$v1" none 06:00:00 2 12G)"
t0="$(submit test_0 "$validation" rtx_5080 48:00:00 2 12G)"
t1="$(submit test_1 "$validation" rtx_5070_ti 48:00:00 2 12G)"
final="$(submit test_score "$t0:$t1" none 06:00:00 2 12G)"
