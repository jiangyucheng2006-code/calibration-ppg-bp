"""Train one frozen personal-profile candidate on the seen-subject track."""
...
def main() -> None:
    ...
    parser.add_argument("--candidate", choices=list(PERSONAL_PROFILES), required=True)
    parser.add_argument("--store-root", type=Path, required=True)
    parser.add_argument("--split-mode", choices=PROFILE_SPLIT_MODES, required=True)
    ...
    result = train_component(
        args,
        components=PERSONAL_PROFILES,
        model_factory=SameSubjectPersonalProfileRegressor,
        screen_id=SCREEN_ID,
        runner="personal_profile_residual",
        allowed_split_modes=PROFILE_SPLIT_MODES,
    )
